...
DECLARE @DH100 AS DECIMAL(6,2)
DECLARE @DH90 AS DECIMAL(6,2)
DECLARE @DH80 AS DECIMAL(6,2)
DECLARE @DH70 AS DECIMAL(6,2)
DECLARE @DH60 AS DECIMAL(6,2)
DECLARE @DH50 AS DECIMAL(6,2)
DECLARE @DH40 AS DECIMAL(6,2)
DECLARE @DH30 AS DECIMAL(6,2)
DECLARE @DH20 AS DECIMAL(6,2)
DECLARE @DH10 AS DECIMAL(6,2)
...
(
	...
	[Open] DECIMAL(6,2),
	[Close] DECIMAL(6,2),
	V100 DECIMAL(8,2) NULL,
	V90 DECIMAL(8,2) NULL,
	V80 DECIMAL(8,2) NULL,
	V70 DECIMAL(8,2) NULL,
	V60 DECIMAL(8,2) NULL,
	V50 DECIMAL(8,2) NULL,
	V40 DECIMAL(8,2) NULL,
	V30 DECIMAL(8,2) NULL,
	V20 DECIMAL(8,2) NULL,
	V10 DECIMAL(8,2) NULL 
);
...
FROM [Barchart].[dbo].[ZacksRank] Where Symbol = @Symbol And [DATE] >= DATEADD(DAY, -100, GETDATE()) Order By DayHigh
...
BEGIN
 ...
 BEGIN
	...
	PRINT 'DayHigh ' + Cast(@DayHigh AS NVARCHAR(MAX))
 ...
 --PRINT @ID
 --PRINT CAST(@DH100 AS NVARCHAR(MAX)) 
 --PRINT CAST(@DH90 AS NVARCHAR(MAX)) 
 --PRINT CAST(@DH80 AS NVARCHAR(MAX)) 
 --PRINT CAST(@DH70 AS NVARCHAR(MAX)) 
 --PRINT CAST(@DH60 AS NVARCHAR(MAX)) 
 --PRINT CAST(@DH50 AS NVARCHAR(MAX))
 --PRINT CAST(@DH40 AS NVARCHAR(MAX)) 
 --PRINT CAST(@DH30 AS NVARCHAR(MAX)) 
 --PRINT CAST(@DH20 AS NVARCHAR(MAX)) 
 --PRINT CAST(@DH10 AS NVARCHAR(MAX))
 ...
 PRINT @DayHigh
 END
 UPDATE #Base SET V100 = ((@Close - @Open) * 100.00) Where ID = @ID
 IF @DayHigh >= @DH100 AND @DH100 > 0 UPDATE #Base SET V100 = (@DH100 * 100.00) Where ID = @ID
